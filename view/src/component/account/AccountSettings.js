...
const AccountSettings = ({auth : {user}, updateUser, setAlert}) => {
    ...
    return (
        ...
        <form className="form form-user-data" onSubmit={onSubmit}>
            ...
            {user && (user.photo ? (<img className="form__user-photo" src={`/photo/user/${user.photo}`} alt={`${user.name} photos`} />) : (<img className="form__user-photo invert" src={userIcon} alt="default user icon"/>))}
